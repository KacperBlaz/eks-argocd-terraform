resource "kubectl_manifest" "eks-service-account" {
  yaml_body = <<YAML
apiVersion: v1
kind: ServiceAccount
metadata:
  annotations:
    ...
    eks.amazonaws.com/role-arn: "${var.sa-role-arn}"
  name: "${var.eks_serviceaccount_name}"
  namespace: "${var.serviceaccount_namespace}"
YAML
}

#resource "kubectl_manifest" "app-namespace" {
#  yaml_body = <<YAML
#apiVersion: v1
#kind: Namespace
#metadata:
...
#}
#
#resource "kubectl_manifest" "loadbalancer-controllerr" {
#  yaml_body = <<YAML
...
#        - --ingress-class=alb
#        image: amazon/aws-alb-ingress-controller:v2.2.4
#        livenessProbe:
#          failureThreshold: 2
#          httpGet:
#            path: /healthz
